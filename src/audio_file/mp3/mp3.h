/**
 ******************************************************************************
 * @file    mp3.h
 * @brief   MP3 processing header.
 ******************************************************************************
 */

#pragma once

#include <string>
#include <vector>

#include "channel.h"
#include "minimp3.h"
#include "minimp3_ex.h"

struct MP3Data {
  /** @brief The number of sample of MP3 data. */
  size_t numSamples;

  /** @brief Channel classification. */
  Channel channel;

  /** @brief Sample rate of the MP3 data. */
  int sampleRate_hz;

  /** @brief Hold PCM data of MP3. Holds left channel if stereo. */
  std::vector<double> channel1;

  /** @brief Hold PCM data of MP3. Holds right channel if stereo. */
  std::vector<double> channel2;
};

/**
 * @brief Reads and processes MP3 file.
 *
 * @param[in] filename path to MP3 file. This is relative to where the binary is
 * being ran.
 * @return MP3Data PCM of MP3 file data.
 */
MP3Data readMP3File(std::string filepath);

/**
 * @brief Read MP3 file and return raw binary contents.
 *
 * @param[in] filepath path to MP3 file. This is relative to where the binary is
 * being ran.
 * @return std::vector<unsigned char> byte MP3 data from file.
 */
std::vector<unsigned char> readRawMP3(std::string filepath);

/**
 * @brief Handles mono channel MP3 processing.
 *
 * @param[in] info MP3 file info indentifies by minimp3 library.
 * @return MP3Data PCM of MP3 file data.
 */
MP3Data handleMonoChannel(mp3dec_file_info_t& info);

/**
 * @brief Handles stereo channel MP3 processing.
 *
 * @param[in] info MP3 file info indentifies by minimp3 library.
 * @return MP3Data PCM of MP3 file data.
 */
MP3Data handleStereoChannel(mp3dec_file_info_t& info);
